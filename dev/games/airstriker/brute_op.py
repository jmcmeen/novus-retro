...
Ursa Blue 2020
-----------------------------------------------------------
Updated retro.openai-retro-examples.brute included with OpenAI Gym Retro

To Render
python -m retro.scripts.playback_movie 780.0best.bk2
-----------------------------------------------------------
...
import numpy as np
import retro
import retro.scripts.playback_movie
import gym
...
def brute_retro(
    ...
    f = open("output.csv", "w")
    f.write("method,game,iter,reward\n")
    f.close()
    ...
    while True:
        ...
        print("remy the brute -> iteration {} scored {} points.".format(counter, rew))
        f.write("{},{},{},{}\n".format("brute", "airstriker", counter, rew))
        f.close()

        if rew > best_rew:
            print("  new best reward {} => {}".format(best_rew, rew))
            best_rew = rew
            env.unwrapped.record_movie("rew{}iter{}.bk2".format(rew, counter))
