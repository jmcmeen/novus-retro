"""
Project: Playing Space Harrier 2 with Gym Retro and NEAT threaded
...
if __name__ == '__main__':
    ...
    # population = neat.Checkpointer.restore_checkpoint('neat-checkpoint-98')
    population.add_reporter(neat.StdOutReporter(True))
    stats = neat.StatisticsReporter()
    population.add_reporter(stats)
    population.add_reporter(neat.Checkpointer(20))

    # create a parallel evaluator that will spawn workers
    pe = neat.ParallelEvaluator(multiprocessing.cpu_count(), eval_genome)

    # the winning network up to x generations
    winner = population.run(pe.evaluate, 50000)

    # save the winning network to a binary file to reload later
    with open('winner-50000gen.pkl', 'wb') as output:
